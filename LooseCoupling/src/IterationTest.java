...
class EventAgency {
	public static Magician getMagician(String hint) {
		...
		else if(hint.equalsIgnoreCase("animal")) {
			magician=new AnimalMagician();
		}
		return magician;
	}
}
...
class AnimalMagician implements Magician{
	public Magic doTrick(Account accRef) {
		System.out.println("Hiding the dove undr the coat....");
		Magic magic = new HatMagic();
		accRef.deposit(6000);
		return magic;
	}
...
class AnimalMagic implements Magic{
	public void show() {
		System.out.println("Replacing rabbit with dove.............");
	}
}
